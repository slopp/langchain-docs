...
See the [`NVIDIAEmbeddings` integration page](/oss/integrations/text_embedding/nvidia_ai_endpoints) for full documentation.
...
`NVIDIARetriever` connects LangChain to a running [NVIDIA RAG Blueprint](https://docs.nvidia.com/rag/latest/index.html) server and retrieves relevant documents via the `/v1/search` endpoint. It supports reranking, query rewriting, and metadata filtering.
...
See the [`NVIDIARetriever` integration page](/oss/integrations/retrievers/nvidia) for full documentation.
...
- [`NVIDIARetriever`](/oss/integrations/retrievers/nvidia)
